...
import numpy as np
from TF2_DDPG_Basic import *
from Prioritized_Replay import ReplayMemory
...
Hector.load_actor("/home/paul-antoine/workspaceRos/src/docking/tensor_models/hector_actor/variables/variables")
Hector.load_critic("/home/paul-antoine/workspaceRos/src/docking/tensor_models/hector_critic/variables/variables")
reached=0
episode=0
...
################################################################Reinforcement################################################################      
            ...
            #f=10
            #hr=-(abs_velocity_angular*1/(1.2*f))
            ...
            #end the simulation
            if reached >=200:
                Hector.save_model("/home/paul-antoine/workspaceRos/src/docking/tensor_models/hector_actor","/home/paul-antoine/workspaceRos/src/docking/tensor_models/hector_critic")
                break
            ...
            with summary_writer.as_default():
                if len(Hector.memory) > Hector.batch_size:
                    ...
                    for t in Hector.summaries['actor_grad']:
                        tf.summary.histogram(str(count), data=t,step=episode)
                        count+=1
